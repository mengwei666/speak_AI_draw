...
def update_settings():
    global wake_word, image_resolution, model_id
    data = request.json
    ...
    <head>
        ...
        <style>
            ...
            #loader {
                ...
                background-size: contain; /* 保持背景图原始宽高比并适应容器 */
        ...
        <script>
            ...
            function fetchTranscriptions() {
                ...
                .then(data => {
                    ...
                    if (data.image_url) {
                        const currentImageUrl = data.image_url; // 更新当前图片URL
                        image.style.display = 'none'; // 隐藏图片
                        image.style.opacity = 0; // 设置初始透明度为0
                        ...
                        image.onload = function() {
                            ...
                            image.src = currentImageUrl; // 设置图片源
                            setTimeout(() => {
                                image.style.opacity = 1; // 渐显图片
                            }, 0);
                        };
                        image.src = currentImageUrl; // 触发图片加载
                    } else {
                        loader.style.display = 'block'; // 显示加载器
                        image.style.display = 'none'; // 隐藏图片
                        image.style.opacity = 0; // 重置图片透明度
                    }
                })
            ...
            document.getElementById('modelId').addEventListener('change', function() {
                ...
                const imageWidth = document.getElementById('imageWidth').value;
                const imageHeight = document.getElementById('imageHeight').value;
                const modelId = this.value; // 获取model_id的值
                updateSettings(wakeWord, imageWidth, imageHeight, modelId);
            });

            function updateSettings(wakeWord, imageWidth, imageHeight, modelId) {
                fetch('/update_settings', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                    },
                    body: JSON.stringify({ wakeWord, imageWidth, imageHeight, modelId }),
                })
                .then(response => response.json())
                .then(data => {
