    ...
    <body>
        ...
        <script>
            ...
            function resetSettingsTimeout() {
                ...
                settingsTimeout = setTimeout(hideSettings, 5000); // 5秒后隐藏
            }

            document.getElementById('settings').addEventListener('mouseenter', showSettings);
            document.getElementById('settings').addEventListener('mouseleave', hideSettings);
            document.getElementById('settings').addEventListener('mousemove', resetSettingsTimeout);

            // 初始化时隐藏设置面板
            document.getElementById('settings').style.display = 'none';

            // 鼠标移动到页面时显示设置
            document.body.addEventListener('mousemove', showSettings);
            ...
            function fetchTranscriptions() {
                ...
                .then(data => {
                    console.log('Image URL:', data.image_url); // 打印 image_url 的内容            
                    if (data.image_url) {
                        console.log('#1'); // 打印 image_url 的内容              
                    ...
                    {
                        console.log('#2'); // 打印 image_url 的内容    
                        const image = document.getElementById('image');
                        image.style.display = 'block'; // 显示图片
                        image.src = currentImageUrl;   
                        // 使图片透明然后恢复
            ...
            // 使图片透明然后恢复
            function fadeImage(imageElement) {
                imageElement.style.transition = 'opacity 0.5s'; // 设置过渡效果
                imageElement.style.opacity = '0'; // 使图片透明

                setTimeout(() => {
                    imageElement.style.opacity = '1'; // 恢复不透明
                }, 250); // 1秒后恢复不透明
            }
